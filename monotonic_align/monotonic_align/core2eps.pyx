...
cdef void maximum_path_each2(int[:,::1] path, float[:,::1] value, int n_symbols, int n_frames, float max_neg_val) noexcept nogil:
  cdef int x
  cdef int y
...
@cython.boundscheck(False)
@cython.wraparound(False)
cpdef void maximum_path_c2eps(int[:,:,::1] paths, float[:,:,::1] values, int[::1] t_xs, int[::1] t_ys, float max_neg_val=-1e9) noexcept nogil:
